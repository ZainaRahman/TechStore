    ...
    <div class="page-wrapper">
        <div class="dashboard-header">
            <div>
                ...
                <p class="welcome-text">Here is your order history and review area</p>
            </div>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-number">{{ count($reviewOrders) }}</div>
            </div>
            <div class="stat-card">
                <h3>Review Items</h3>
                <div class="stat-number">{{ collect($reviewOrders)->sum(fn ($order) => count($order->items)) }}</div>
            </div>
        </div>
        ...
        <div class="dashboard-section">
            ...
            @if (empty($reviewOrders))
                <p class="card-subtext">Once an order is marked delivered, you can review its products here.</p>
            @else
                                        ...
                                        <div class="review-item-content">
                                            <div class="review-item-topline">
                                                <div>
                                                    ...
                                                    <p class="product-stock">{{ $item->stock_quantity }} left in stock</p>
                                                </div>
                                                @if ($item->review_id)
                                            ...
                                            <p class="card-subtext">Ordered quantity: {{ $item->order_quantity }}</p>

                                            @if ($item->review_id)
                                                <div class="review-existing">
                                                    <strong>Your rating:</strong> {{ $item->review_rating }} / 5
                                                    @if ($item->review_comment)
                                                        <p>{{ $item->review_comment }}</p>
                                                    @endif
                                                </div>
                                            @else
                                                <form method="POST" action="{{ route('reviews.store', $item->product_id) }}" class="review-form">
                                                    @csrf
                                                    <div class="field">
                                                        <label>Rating</label>
